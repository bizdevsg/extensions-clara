...
const parsePrePlainText = (value: string) => {
  ...
  const match = trimmedValue.match(/^\[(?<timestamp>[^\]]+)\]\s?(?<author>.*?)(?::)?$/)
...
const readOpenChat = (): WhatsAppReadResponse => {
...
const insertReplyIntoComposeBox = (text: string): WhatsAppActionResponse => {
  const chatRoot = getChatRoot()
  ...
  try {
    insertedWithNativeCommand = document.execCommand("insertText", false, normalizedText)
  } catch (_error) {
